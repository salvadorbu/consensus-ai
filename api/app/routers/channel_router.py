...
def _run_channel(channel_id: str, loop: asyncio.AbstractEventLoop) -> None:
...
async def create_channel(req: CreateChannelRequest, background_tasks: BackgroundTasks, session: AsyncSession = Depends(get_session)):
    ...
    background_tasks.add_task(loop.run_in_executor, _executor, _run_channel, channel_id, loop)
